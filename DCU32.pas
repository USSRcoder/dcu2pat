...
begin
  ...
  addr := 0;
  if (D is TTypeDecl)or(D is TTypeDef) then begin
    //TDCURec(TFPList(TList(FAddrs).FList).List^[hDef-1]).
  end ;
  if Pfx<>'' then begin
...
begin
 ...
 (*
  ...
  if ValPtr=Nil then begin
    ...
    Inc(NLOfs,2);
    NL;
    ShowDump(ValPtr,0,ValSz,0,0,0,0,Nil,false,Nil,Nil);
    Dec(NLOfs,2);
  end ;
  if TypeNamed then
    PutS(')');
...
//ida crc16
function crc16(data_p: PByte; length:integer):word;
var
  i: byte;
  crc: DWORD;
  data: WORD;
  p:integer = 0;
const POLY:word=$8408;
begin
...
//    PutS('; ');
    ...
    PutS('begin');
    NLOfs := Ofs0+2;
    CurUnit.ShowCodeBl(AddrBase,CodeOfs,Sz, OutS, OutPat);

    //test := chr($E1)+chr($50)+chr($56)+chr($FF)+chr($33)+chr($E8);
    //w := crc_16(PByte(t),6);
    ...
    c16 := 0;
    cl := 0;
    if (slen >= 64) then begin

      test := '';
      //HexStr := 'E15056FF33E8[';
      HexStr:=Copy(OutS,65,slen - 64);
      //Writeln;Writeln('Hex=',HexStr);
      i := Low(HexStr);
      en := High(HexStr);
      while i <= en do begin
          if (HexStr[i] = '[') or (HexStr[i] = '.') or (i-1 = 255*2) then break;
          h := Copy(HexStr, i, 2);
          nr := StrToInt('$' + h);
          test += char(nr);
          Inc(i, 2);
      ...
      cl := i >> 1;
      if cl > 0 then
         c16 := crc16(PByte(t),cl);
      //WriteLn('crc = ',Format('%4.4X',[crc16(PByte(t),6)])); //crc
    end;

    Write(FNPat,' '+Format('%2.2X',[cl])); //crc size
    Write(FNPat,' '+Format('%4.4X',[c16])); //crc16

    Write(FNPat,' ',Format('%4.4X',[Sz])); //size
    if Def^.Name = '.' then
       Write(FNPat,' :0000 ?') //name
    else
       Write(FNPat,' :0000 ',Format('%s',[Def^.Name])); //name

    Write(FNPat,' ',Trim(OutPat)); //pat
    if (slen >= 64) then begin
        Write(FNPat,' ',Copy(OutS,65 + cl *2 ,slen - 64 - cl *2 )); //pat
    end;
    WriteLn(FNPat);

    end; // slen > 0
    WriteLn('lala9:',OutPat, ' ',OutS);
    Flush(Output);
...
begin
  ...
  if (PChar(DP)>=FDataBlPtr)and(PChar(DP)<FDataBlPtr+FDataBlSize) then
    ...
    OutS := '';
    OutP := '';
    ShowDump(DP,0,DS,SzShown,SzShown,0,0,Nil,false, OutS, OutP);
  end ;
end ;
...
begin
  ...
  if BlOfs<>Cardinal(-1) then
    PutSFmt('at 0x%x',[(FDataBlPtr - FMemPtr + BlOfs )]);

  //if (FDataBlPtr - FMemPtr + BlOfs) = $B04D then
  //   DebugBreak;
...
//  Inc(NLOfs,2);
  ...
  while true do begin
    ...
    ShowDump(FDataBlPtr+CmdOfs,BlSz+Ofs0,CmdSz,CmdOfs-BlOfs+Ofs0,CmdOfs,
      7,FFixupCnt-Fix0,@FFixupTbl^[Fix0],not Ok,DmpS,OutP);
    ...
    if not Ok then begin
      ...
      if (OutP <> '') then begin
        ...
        if (p1 > 0) and (p2 > 0) then begin
          OutPat += (copy(OutP,1,p1) + copy(CmdS,1,p2));
          OutPat += (copy(OutP,p1+2,255) + copy(CmdS,p2+2,255));
        end else begin
          OutPat += (OutP + CmdS);
          if (CmdS='')and(OutP<>'') then
             OutPat += '?';
        end;
      end;

      DmpS:=StringReplace(DmpS,'[','.',[rfReplaceAll]);
      DmpS:=StringReplace(DmpS,']','.',[rfReplaceAll]);
      OutS+= DmpS;
    end ;
