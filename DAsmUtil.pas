...
function WriteBMOpName_pat(hN: THBMName):string;
begin
  result := GetOpName(hN);
end ;
...
function WriteInt_pat(i:integer):string;
begin
  result := PutSFmt_pat('%d',[i]);
end ;
...
procedure WriteImmed_pat(hDSize,Ofs:Byte; MayBeAddr: boolean; Fix: PFixupRec; var OutS:string);
var
  DP,DP1: Pointer;
  IsAddr: boolean;
  A: Pointer;
...
begin
  DP := PrevCodePtr+Ofs;
  Fixed := ReportFixUp_pat(Fix,OutS);
    ...
    //  OutS+=PutS_pat('+');
    Case hDSize and dsMask of
      dsByte:; //OutS+=PutSFmt_pat('$%2.2x',[Byte(DP^)]);
      dsWord:; //OutS+=PutSFmt_pat('$%4.4x',[Word(DP^)]);
      dsDbl: begin
          V := LongInt(DP^);
          if Fixed then begin
            Fixed := V=0;
          ...
          //if not Fixed then
          //  OutS+=PutSFmt_pat('$%8.8x',[V]);
        end ;
      dsPtr:;//  if not OpIs32 then
             //   OutS+=PutSFmt_pat('$%8.8x',[LongInt(DP^)])
             // else begin
             //   DP1 := DP;
             //   Inc(integer(DP1),4);
             //   OutS+=PutSFmt_pat('$%4.4x:$%8.8x',[Word(DP1^),LongInt(DP^)]);
             // end ;
      dsQWord:;// OutS+=PutS_pat(CharDumpStr(DP^,8));
      dsTWord:;// OutS+=PutS_pat(CharDumpStr(DP^,10));
    else
      //OutS+=PutS_pat('?Immed');
    End ;
end ;
...
function WriteIntData_pat(SignRq,FixSignRq: boolean;hDSize,Ofs:Byte; Fix: PFixupRec; var OutS : String): LongInt;
var
  DP: Pointer;
  DOfs: LongInt;
  Fixed: boolean;
...
begin
  ...
  //  OutS += PutS_pat('+');
  Case hDSize and dsMask of
    dsByte:; //OutS += WriteInt_pat(ShortInt(DP^));
    dsWord:; //OutS += WriteInt_pat(SmallInt(DP^));
    dsDbl: begin
      V := LongInt(DP^);
      if Fixed then begin
        Fixed := V=0;
        //if not Fixed then OutS += PutS_pat('+');
      end ;
      //if not Fixed then OutS += WriteInt_pat(LongInt(DP^));
    end ;
  End ;
  WriteIntData_pat := DOfs;
end ;
...
procedure WriteJmpOfs_pat(hDSize,Ofs:Byte; Fix: PFixupRec; var OutS:String);
var
  DOfs: LongInt;
begin
  //DOfs := WriteIntData_pat(true,false,hDSize,Ofs,Fix, OutS);
  DOfs := WriteIntData_pat(false,false,hDSize,Ofs,Fix, OutS);
  if Fix=Nil then begin
    //OutS += PutS_pat('; (');
    //OutS += PutSFmt_pat('0x%x',[(CodePtr-CodeBase)+DOfs]);
    //OutS += PutS_pat(')');
  end ;
end ;
...
function ReportImmed_pat(IsInt,SignRq: boolean;DSF,hDSize,SegN,Ofs:Byte;
  Fix: PFixupRec; var OutS:String): boolean;
var
  RepRes: Byte;
begin
  {if ReportDataRefsOn then
  ...
  //  if SignRq then
  //    OutS+=PutS_pat('+');
    WriteImmed_pat(hDSize,Ofs,RepRes>0,Fix,OutS);
   end
  else
    WriteIntData_pat(SignRq,true,hDSize,Ofs,Fix,OutS);
  //if RepRes>1 then
  //  OutS+=PutS_pat('}');
  ReportImmed_pat := RepRes>1;
end ;
...
procedure WriteEA_pat(var OutS:String);
var
  SegN,DSF: Byte;
  Cnt:integer;
  RepRes: boolean;
  ...
  begin
    ...
    Plus;
    //if (SS>0)and(SS<=3) then begin
    //  OutS+=PutS_pat(ScaleStr[SS]);
    //end ;
    //OutS+=WriteBMOpName_pat(RegTbl[1+Ord(AdrIs32)]^[hReg and $7]);
  end ;
...
begin
  ...
  //Case DSF of
  //  0:;
  //  dsByte: OutS+=PutS_pat('BYTE');
  //  dsWord: OutS+=PutS_pat('WORD');
  //  dsDbl:  OutS+=PutS_pat('DWORD');
  //  dsPtr:  OutS+=PutS_pat('DWORD');
  //  dsQWord:OutS+=PutS_pat('QWORD');
  //  dsTWord:OutS+=PutS_pat('TWORD');
  //else
  //  OutS+=PutS_pat('?');
  //End ;
  //if DSF<>0 then
  ...
  SegN := Cmd.EA.hSeg and $f;
  //if SegN<hDefSeg then begin
  //  OutS+=WriteBMOpName_pat(SegRegTbl^[segN]);
  //  OutS+=PutS_pat(':');
  //end ;
  Cnt := 0;
  //OutS+=PutS_pat('[');
  ...
  WriteReg(Cmd.EA.hBase shr 4,Cmd.EA.SS);
  if Cmd.EA.dOfs<>0 then
    ReportImmed_pat(Cnt>0,Cnt>0,DSF,
            Cmd.EA.dOfs shr 5,SegN and $7,Cmd.EA.dOfs and $1f,Cmd.EA.Fix, OutS);
  //OutS+=PutS_pat(']');
end ;
...
procedure WriteArg_pat(const A: TCmArg; var OutS:String);
begin
  Case A.Kind and caMask of
    caReg: WriteBMOpName(A.Inf);
    caEffAdr:WriteEA_pat(OutS);
    caVal: PutSFmt('$%x',[A.Inf]);
    caImmed: ReportImmed_pat(false,false,0,A.Kind shr 4,hCS,A.Inf,A.Fix, OutS);
           {WriteImmed(A.Kind shr 4,A.Inf,false);}
    caJmpOfs: WriteJmpOfs_pat(A.Kind shr 4,A.Inf,A.Fix, OutS);
    caInt: ReportImmed(true,false,0,A.Kind shr 4,hCS,A.Inf,A.Fix);
           {WriteIntData(false,falseA.Kind shr 4,A.Inf);}
  else
    PutS('?');
...
//  ReportCommandMem;
  ...
  for i:=1 to Cmd.Cnt do begin
    ...
    //if (OpName = 'MUL ') then begin
    //  WriteArg_pat(Cmd.Arg[i],result);
    //  WriteLn('lala0:',result);
    //  Flush(Output);
    //end;
    //if (OpName = 'CALL ') then begin
       if (result <> '') then
          result += '  ';
       WriteArg_pat(Cmd.Arg[i],result);
       if (result<>'') then begin
         ...
         if (result = 'ErrorAddr') then begin
           WriteLn('lala1:',result);
         end;

          WriteLn('lala7:',result);
       end;
       Flush(Output);
    //end;
  end ;
