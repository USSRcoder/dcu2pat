...
function ReportFixup_pat(Fix: PFixupRec; var S:string): boolean;
...
begin
  ...
  if (Fix=Nil)or(FixUnit=Nil) then
    Exit;
  Inc(AuxLevel);
  PutSFmt('FK%x ',[TByte4(Fix^.OfsF)[3]]);
  if Fix^.NDX = $101 then begin
     writeln('lala23');
     Flush(Output);
  end;
  Dec(AuxLevel);
...
begin
  ...
  if (Fix=Nil)or(FixUnit=Nil) then
    Exit;
  Inc(AuxLevel);
  //S += PutSFmt_pat('K%x ',[TByte4(Fix^.OfsF)[3]]);
  if Fix^.NDX = $101 then begin
     writeln('lala22');
     Flush(Output);
  end;
  Dec(AuxLevel);
  S+= PutS_pat(TUnit(FixUnit).GetAddrStr(Fix^.NDX,false));
  Result := true;
end ;
